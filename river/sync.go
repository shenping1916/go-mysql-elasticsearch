...
func (h *eventHandler) OnRow(e *canal.RowsEvent) error {
	...
	case canal.InsertAction:
		reqs, err = h.r.makeInsertRequest(rule, e.Rows, e)
	case canal.DeleteAction:
		reqs, err = h.r.makeDeleteRequest(rule, e.Rows, e)
	case canal.UpdateAction:
		reqs, err = h.r.makeUpdateRequest(rule, e.Rows, e)
	default:
...
func (r *River) makeRequest(rule *config.Rule, action string, rows [][]interface{}, e *canal.RowsEvent) ([]*elastic.BulkRequest, error) {
	...
	for _, values := range rows {
		...
		req := &elastic.BulkRequest{Index: rule.Index, Type: rule.Type, ID: id, Parent: parentID, Pipeline: rule.Pipeline, Event: e}
...
func (r *River) makeInsertRequest(rule *config.Rule, rows [][]interface{}, e *canal.RowsEvent) ([]*elastic.BulkRequest, error) {
	return r.makeRequest(rule, canal.InsertAction, rows, e)
}

func (r *River) makeDeleteRequest(rule *config.Rule, rows [][]interface{}, e *canal.RowsEvent) ([]*elastic.BulkRequest, error) {
	return r.makeRequest(rule, canal.DeleteAction, rows, e)
}

func (r *River) makeUpdateRequest(rule *config.Rule, rows [][]interface{}, e *canal.RowsEvent) ([]*elastic.BulkRequest, error) {
	if len(rows)%2 != 0 {
	...
	for i := 0; i < len(rows); i += 2 {
		...
		req := &elastic.BulkRequest{Index: rule.Index, Type: rule.Type, ID: beforeID, Parent: beforeParentID, Event: e}

		if beforeID != afterID || beforeParentID != afterParentID {
			req.Action = elastic.ActionDelete
			reqs = append(reqs, req)

			req = &elastic.BulkRequest{Index: rule.Index, Type: rule.Type, ID: afterID, Parent: afterParentID, Pipeline: rule.Pipeline, Event: e}
...
func (r *River) makeInsertReqData(req *elastic.BulkRequest, rule *config.Rule, values []interface{}) {
...
func (r *River) makeUpdateReqData(req *elastic.BulkRequest, rule *config.Rule,
...
func (r *River) getDocID(rule *config.Rule, row []interface{}) (string, error) {
	var (
		ids []interface{}
		err error
	)
...
func (r *River) getParentID(rule *config.Rule, row []interface{}, columnName string) (string, error) {
