...
func (s *riverTestSuite) SetUpSuite(c *C) {
	...
	cfg.BulkSize = 1
	cfg.FlushBulkTime = config.TomlDuration{3 * time.Millisecond}

	os.RemoveAll(cfg.DataDir)

	cfg.Sources = []config.SourceConfig{config.SourceConfig{Schema: "test", Tables: []string{"test_river", "test_river_[0-9]{4}", "test_for_id", "test_for_json"}}}
